...
<div class="page-header">
    ...
    <a href="/crm_einsurglobal/public/accounts/create" class="btn btn-primary">
...
<div class="card">
    <div style="padding: 1.5rem; border-bottom: 1px solid var(--border); display: flex; gap: 1rem;">
        <form action="/crm_einsurglobal/public/accounts" method="GET" style="display: flex; gap: 1rem; flex: 1;">
            ...
            <?php if (!empty($_GET['search']) || !empty($_GET['type'])): ?>
                <a href="/crm_einsurglobal/public/accounts" class="btn" style="background: var(--border); color: var(--text-main);">Limpiar</a>
            <?php endif; ?>
        </form>
    </div>
                            ...
                            <td>
                                <div style="display: flex; gap: 0.5rem; align-items: center;">
                                    <?php if (\App\Core\Permission::has('accounts', 'update')): ?>
                                        <a href="/crm_einsurglobal/public/accounts/edit?id=<?= $account->id ?>" style="color: var(--primary-hover); text-decoration: none; font-weight: 600;">Editar</a>
                                    <?php endif; ?>
                                    <?php if (\App\Core\Permission::has('accounts', 'delete')): ?>
                                        <form action="/crm_einsurglobal/public/accounts/delete" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta organización?');" style="display:inline; margin:0;">
