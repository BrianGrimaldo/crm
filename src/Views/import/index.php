...
.import-hero h1 { font-size: 2rem; font-weight: 800; margin-bottom: 0.4rem; letter-spacing: -0.04em; }
.import-hero p  { font-size: 1rem; opacity: 0.8; max-width: 640px; }

/* ═══ TABS ═══ */
.tab-bar { display: flex; gap: 0.5rem; margin-bottom: 1.5rem; }
.tab-btn {
    padding: 0.65rem 1.4rem; border-radius: 12px; border: 2px solid var(--border);
    background: var(--surface); color: var(--text-muted); font-weight: 700; font-size: 0.9rem;
    cursor: pointer; transition: all 0.25s; display: flex; align-items: center; gap: 0.5rem;
}
.tab-btn:hover  { border-color: var(--primary); color: var(--primary); }
.tab-btn.active { background: var(--primary); border-color: var(--primary); color: #fff; box-shadow: 0 4px 14px rgba(0,45,98,0.25); }

/* ═══ GRID ═══ */
.import-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; }
@media(max-width:960px){ .import-grid { grid-template-columns: 1fr; } }
...
.drop-zone {
    ...
    transition: all 0.3s ease; position: relative; background: #fafbfc;
}
...
.drop-zone .dz-icon  { font-size: 3rem; color: var(--border); margin-bottom: 1rem; transition: color 0.3s; }
.drop-zone:hover .dz-icon { color: var(--primary); }
.drop-zone h3 { font-weight: 700; font-size: 1.05rem; color: var(--text-main); margin-bottom: 0.3rem; }
.drop-zone p  { font-size: 0.85rem; color: var(--text-muted); }
.drop-zone input[type="file"] { display: none; }
.file-selected { font-size: 0.82rem; color: #10b981; font-weight: 700; margin-top: 0.75rem; }
...
.col-badge.required { background: rgba(0,45,98,0.07); color: var(--primary); border-color: rgba(0,45,98,0.15); }

/* ═══ PROGRESS ═══ */
.progress-wrap { margin-top: 1rem; }
.progress-bar-bg { height: 8px; background: rgba(0,0,0,0.06); border-radius: 8px; overflow: hidden; }
.progress-bar-fill { height: 100%; background: linear-gradient(90deg, var(--primary), var(--accent)); border-radius: 8px; transition: width 0.6s ease; width: 0%; }
.progress-label { font-size: 0.8rem; color: var(--text-muted); font-weight: 600; margin-top: 0.4rem; }

/* ═══ PREVIEW TABLE ═══ */
.preview-wrap { overflow-x: auto; margin-top: 1.5rem; border-radius: 14px; border: 1px solid var(--border); }
.preview-tbl { width: 100%; border-collapse: collapse; font-size: 0.82rem; }
.preview-tbl th {
    background: #f8fafc; padding: 0.7rem 1rem; font-size: 0.72rem;
    text-transform: uppercase; letter-spacing: 0.05em;
    color: var(--text-muted); font-weight: 800; border-bottom: 1px solid var(--border);
    white-space: nowrap;
}
.preview-tbl td { padding: 0.65rem 1rem; border-bottom: 1px solid rgba(0,0,0,0.04); color: var(--text-main); white-space: nowrap; }
.preview-tbl tr:last-child td { border-bottom: none; }
.preview-tbl tr:hover td { background: #fafbff; }
...
.result-box h3 { font-weight: 800; font-size: 1rem; margin-bottom: 0.5rem; }
.result-box.success h3 { color: #166534; }
.result-box.error   h3 { color: #991b1b; }
.result-stats { display: flex; gap: 1rem; flex-wrap: wrap; margin-top: 0.75rem; }
.stat-pill {
    display: flex; align-items: center; gap: 0.4rem;
    background: rgba(255,255,255,0.7); border-radius: 10px;
    padding: 0.4rem 0.8rem; font-size: 0.85rem; font-weight: 700;
}
...
.step-body h4 { font-weight: 700; font-size: 0.92rem; color: var(--text-main); margin-bottom: 0.2rem; }
.step-body p  { font-size: 0.82rem; color: var(--text-muted); line-height: 1.5; }
...
.btn-import:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,45,98,0.3); }
...
.btn-commit:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(16,185,129,0.4); }
.btn-commit.visible { display: flex; }
</style>

<!-- Hero -->
<div class="import-hero">
    <h1><i class="fas fa-file-import" style="margin-right:0.6rem;"></i> Importación Masiva de Datos</h1>
    <p>Carga tu lista de contactos u organizaciones desde un archivo CSV y el sistema los registrará automáticamente en tu cuenta.</p>
</div>
...
<div class="import-grid">
    ...
    <div>
        <div class="panel-card">
            <div class="panel-card-title">
                <div class="icon" style="background:rgba(99,102,241,.1);color:#6366f1;"><i class="fas fa-upload"></i></div>
                Cargar archivo CSV
            </div>
        ...
        <div id="previewSection" style="display:none; margin-top:1.5rem;">
            <div class="panel-card">
                <div class="panel-card-title">
                    <div class="icon" style="background:rgba(16,185,129,.1);color:#10b981;"><i class="fas fa-table"></i></div>
                    Vista previa <span id="previewCount" style="font-size:0.78rem;font-weight:600;color:var(--text-muted);margin-left:0.5rem;"></span>
                </div>
    ...
    <div style="display:flex;flex-direction:column;gap:1.5rem;">
        ...
        <div class="panel-card">
            <div class="panel-card-title">
                <div class="icon" style="background:rgba(245,158,11,.1);color:#f59e0b;"><i class="fas fa-list-ol"></i></div>
                ¿Cómo funciona?
            </div>
            <div class="steps">
                <div class="step">
                    ...
                    <div class="step-body">
                        ...
                        <p>Usa nuestra plantilla para asegurarte de que las columnas estén correctas. Excel abrirá el CSV sin problemas.</p>
                    </div>
                </div>
                <div class="step">
                    ...
                    <div class="step-body">
                        ...
                        <p>Agrega tus contactos u organizaciones. La columna marcada con * es obligatoria; el resto son opcionales.</p>
                    </div>
                </div>
                <div class="step">
                    ...
                    <div class="step-body">
                        ...
                        <p>En Excel: <em>Guardar como → CSV UTF-8 (delimitado por comas)</em>. En Google Sheets: <em>Archivo → Descargar → CSV</em>.</p>
                    </div>
                </div>
                <div class="step">
                    ...
                    <div class="step-body">
                        ...
                        <p>Sube el archivo, revisa la preview y presiona <strong>Confirmar e importar</strong>. ¡Listo!</p>
                    </div>
                </div>
            </div>
            ...
            <div style="display:flex;flex-direction:column;gap:0.5rem;margin-top:1.5rem;">
                <a id="dlContacts" href="/importar/plantilla?type=contacts"
                   class="btn btn-primary" style="justify-content:center;font-size:0.9rem;">
                    <i class="fas fa-download"></i> Plantilla de Contactos
                </a>
                <a id="dlAccounts" href="/importar/plantilla?type=accounts"
                   style="display:none; justify-content:center;font-size:0.9rem;"
        ...
        <div class="panel-card">
            <div class="panel-card-title">
                <div class="icon" style="background:rgba(168,85,247,.1);color:#a855f7;"><i class="fas fa-columns"></i></div>
            ...
            <div id="colContacts">
                <p style="font-size:0.8rem;color:var(--text-muted);margin-bottom:0.75rem;">
                    <span style="background:rgba(0,45,98,0.08);color:var(--primary);padding:0.15rem 0.5rem;border-radius:6px;font-weight:700;font-size:0.72rem;">*obligatorio</span>
                </p>
                <div class="col-grid">
...
async function uploadPreview() {
    ...
    try {
        const res = await fetch('/importar/preview', { method: 'POST', body: form });

        // Check if response is OK
        if (!res.ok) {
            const text = await res.text();
            setProgress(false);
            showResult('error', '❌ Error del servidor (' + res.status + ')', text.substring(0, 300));
            return;
        }
...
async function commitImport() {
    ...
    try {
        const res = await fetch('/importar/commit', { method: 'POST' });
...
function showResult(type, title, message, data=null) {
    ...
    if (data) {
        ...
        // Link to destination
        const dest = currentType === 'contacts' ? '/contactos' : '/organizaciones';
        const lbl  = currentType === 'contacts' ? 'Ver Contactos' : 'Ver Organizaciones';
