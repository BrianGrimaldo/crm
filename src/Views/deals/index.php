...
<div class="card" style="box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border-radius: 16px;">
    <div
        style="padding: 1.5rem; border-bottom: 1px solid var(--border); display: flex; gap: 1rem; background: var(--surface); border-top-left-radius: 16px; border-top-right-radius: 16px;">
        <form action="<?= url('/oportunidades') ?>" method="GET"
            style="display: flex; gap: 1rem; flex: 1; flex-wrap: wrap;">
            <input type="text" name="search" class="form-control" placeholder="Buscar por nombre o contacto..."
                value="<?= htmlspecialchars($_GET['search'] ?? '') ?>"
                style="background: rgba(0,0,0,0.02); min-width: 200px;">
            <select name="status" class="form-control"
                style="width: auto; min-width: 150px; flex-grow: 1; max-width: 100%; background: rgba(0,0,0,0.02);">
            ...
            <button type="submit" class="btn btn-primary"
                style="border-radius: 8px; flex-grow: 1; justify-content: center;">Buscar</button>
            <?php if (!empty($_GET['search']) || !empty($_GET['status'])): ?>
                <a href="<?= url('/oportunidades') ?>" class="btn"
                    style="background: var(--border); color: var(--text-main); flex-grow: 1; justify-content: center;">Limpiar</a>
            <?php endif; ?>
        </form>
    </div>
                    ...
                    <tr>
                        <td colspan="11" style="text-align: center; padding: 4rem 2rem;">
                            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" style="color: var(--text-muted); opacity: 0.5; margin-bottom: 1rem;">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                            </svg>
                            <p style="color: var(--text-muted); font-size: 1.1rem; font-weight: 500;">No se encontraron
                                oportunidades.</p>
                        </td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr>
                            <td>
                                <a href="<?= url('/oportunidades/edit?id=' . $deal->id) ?>" class="deal-name">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor" style="color: var(--text-muted);">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <td>
                                <div style="display: flex; align-items: center; gap: 0.5rem;">
                                    <div
                                        style="width: 24px; height: 24px; border-radius: 50%; background: var(--border); display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 700;">
                            ...
                            <td>
                                <span class="stage-badge"
                                    style="background: <?= htmlspecialchars($deal->stage_color ?? 'var(--border)') ?>25; color: <?= htmlspecialchars($deal->stage_color ?? 'var(--text-muted)') ?>; border: 1px solid <?= htmlspecialchars($deal->stage_color ?? 'var(--border)') ?>50;">
                                    <?= htmlspecialchars($deal->stage_name ?? 'Desconocida') ?>
                                </span>
                            </td>
                            <td><span
                                    style="font-size: 0.85rem; color: var(--text-muted);"><?= htmlspecialchars($deal->lost_reason ?? '-') ?></span>
                            </td>
                            <td><span
                                    style="font-size: 0.85rem; color: var(--text-muted);"><?= htmlspecialchars($deal->source ?? '-') ?></span>
                            </td>
                            <td>
                            ...
                            <td>
                                <?php if ($deal->probability): ?>
                                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                                        <div
                                            style="flex: 1; background: var(--border); height: 6px; border-radius: 3px; overflow: hidden; width: 50px;">
                                            <div
                                                style="height: 100%; width: <?= $deal->probability ?>%; background: <?= $deal->probability >= 70 ? '#10b981' : ($deal->probability >= 40 ? '#f59e0b' : '#ef4444') ?>;">
                                            </div>
                                        </div>
                            ...
                            <td>
                                <div style="display: flex; gap: 0.5rem; align-items: center;">
                                    ...
                                    <?php if (\App\Core\Permission::has('deals', 'delete')): ?>
                                        <form action="<?= url('/oportunidades/delete') ?>" method="POST"
                                            onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta oportunidad?');"
